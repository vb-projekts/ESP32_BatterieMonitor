...
Changelog:
  ...
  v2.1 - Updatefunktion für die ESP32 Boards
...
# ================================================================
#  Version + Firmwareordner
# ================================================================
FIRMWARE_ORDNER  = "./firmware"   # Ordner wo .bin Dateien liegen
AKTUELLE_VERSION = "2.1"          # Manuell hochsetzen bei neuem Release
...
<div class="footer">
  Raspberry Pi ESP32 Multi-Board Monitor v"""+AKTUELLE_VERSION+""" &bull; Port {{ port }} &bull;
...
# ================================================================
#  API ENDPUNKT - Update -funktionen zu den ESP32 -Boards
# ================================================================
# Endpunkt 1: Versionsprüfung
@app.route("/firmware/version")
def firmware_version():
    return jsonify({
        "version":     AKTUELLE_VERSION,
        "datei":       f"firmware_v{AKTUELLE_VERSION}.bin",
        "url":         f"http://<Pi-IP>:5000/firmware/download"
    })
# Endpunkt 2: .bin Datei herunterladen
@app.route("/firmware/download")
def firmware_download():
    pfad = os.path.join(FIRMWARE_ORDNER,
                        f"firmware_v{AKTUELLE_VERSION}.bin")
    if os.path.exists(pfad):
        return send_file(pfad, mimetype="application/octet-stream")
    return "Firmware nicht gefunden", 404

# Endpunkt 3: Upload neuer Firmware (per Browser)
@app.route("/firmware/upload", methods=["GET", "POST"])
def firmware_upload():
    if request.method == "POST":
        datei = request.files.get("firmware")
        if datei:
            pfad = os.path.join(FIRMWARE_ORDNER, f"firmware_v{AKTUELLE_VERSION}.bin")
            os.makedirs(FIRMWARE_ORDNER, exist_ok=True)
            datei.save(pfad)
            return "Upload erfolgreich!", 200
    # GET: einfaches Upload-Formular
    return """
...
if __name__ == "__main__":
    print("=" * 50)
    print("  ESP32 Multi-Board Monitor Server v"+AKTUELLE_VERSION)
